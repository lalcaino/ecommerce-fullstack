...
export async function registroRequest({ nombre, email, password, rut, razonSocial, giro, region }) {
  const res = await fetch(`${BASE}/registro/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ nombre, email, password, rut, razon_social: razonSocial, giro, region }),
  })
  const data = await res.json()
...
export async function apiFetch(url, options = {}) {
  ...
  // Token expirado → intenta renovar una vez
  if (res.status === 401) {
    try {
