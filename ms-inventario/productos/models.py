...
class ProductoRepository:
    """
    Abstrae toda la lógica de persistencia del modelo Producto.
    Permite cambiar el ORM o la fuente de datos sin afectar las vistas.
    """

    @staticmethod
    def get_all(activo: bool = True):
        return Producto.objects.filter(activo=activo)

    @staticmethod
    def get_by_id(pk: int) -> Producto:
    ...
    @staticmethod
    def update(pk: int, data: dict) -> Producto:
    ...
    @staticmethod
    def ajustar_stock(pk: int, cantidad: int) -> Producto:
        """Ajusta el stock de forma atómica para evitar condiciones de carrera."""
        from django.db.models import F
